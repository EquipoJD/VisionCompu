...
int main() {
	//Lee la imagen
	Mat image1, image2;
	image2 = imread("ratong.jpg", CV_LOAD_IMAGE_GRAYSCALE);
	image1 = imread("testratong.jpg", CV_LOAD_IMAGE_GRAYSCALE);
	if (!image1.data || !image2.data) {
		cout << "Could not open or find the image" << std::endl;
	} else {

		//-- Step 1: Detect the keypoints using SURF Detector
		int minHessianbf = 400;

		SurfFeatureDetector detectorbf(minHessianbf);

		std::vector<KeyPoint> keypoints_1, keypoints_2;

		detectorbf.detect(image1, keypoints_1);
		detectorbf.detect(image2, keypoints_2);

		//-- Step 2: Calculate descriptors (feature vectors)
		SurfDescriptorExtractor extractorbf;

		Mat descriptors_1, descriptors_2;

		extractorbf.compute(image1, keypoints_1, descriptors_1);
		extractorbf.compute(image2, keypoints_2, descriptors_2);

		//-- Step 3: Matching descriptor vectors with a brute force matcher
		BFMatcher matcherbf(NORM_L2, false);
		std::vector<DMatch> matches;
		matcherbf.match(descriptors_1, descriptors_2, matches);

		//-- Draw matches
		Mat img_matches;
		drawMatches(image1, keypoints_1, image2, keypoints_2, matches,
				img_matches);
		...
		while (key != 27) {
			...
			drawMatches(object, kp_object, image, kp_image, good_matches,
